...
C     *==========================================================*
     ...
     &              FIce, Silica, Kwexch_Pre
      ...
      _RL  light(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      _RL  riv_nutr(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      _RL  riv_dis(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
C  dic_int*          :: place holder to read in a integer number, set at run time
     ...
     &        light_file, riv_nutr_file, riv_dis_file,
     &        DIC_ironFile, DIC_silicaFile, DIC_parFile,
     &        DIC_chlaFile,
     &        DIC_forcingPeriod, DIC_forcingCycle,
     &        DIC_forcingPeriod2, DIC_forcingCycle2,
     &        dic_pCO2, dic_int1, dic_int2, dic_int3, dic_int4

      CHARACTER*(MAX_LEN_FNAM) DIC_windFile
      CHARACTER*(MAX_LEN_FNAM) light_file
      CHARACTER*(MAX_LEN_FNAM) riv_nutr_file
      CHARACTER*(MAX_LEN_FNAM) riv_dis_file
      ...
      _RL     DIC_forcingPeriod2
      _RL     DIC_forcingCycle2
      _RL dic_pCO2
      INTEGER dic_int1
      INTEGER dic_int2
      INTEGER dic_int3
